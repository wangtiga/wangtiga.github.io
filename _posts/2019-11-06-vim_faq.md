...
#### TODO vim-go 插件提示 "packages.Load error" `Ctrl + [` 时如何跳转到 vendor 目录的代码？

- 跟 go.mod 的版本有关

  某些项目将 go.mod 文件中更改为 go1.14 后可正常跳转 vendor 目录的代码

  ```txt
  vendor/github.com/spf13/cobra/args.go
  ```

- 部分项目调整 go.mod go1.14 后 提示 "vim-go: : packages.Load error"

  即使更改 go.mod 文件为 go1.13 ，仍然会跳转到 ~/go/pkg/mod 目录的代码

  ```txt
  ~/go/pkg/mod/github.com/spf13/cobra@v1.1.1/args.go
  ```
