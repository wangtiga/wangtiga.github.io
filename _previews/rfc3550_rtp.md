...
Updated by: 5506, 5761, 6051, 6222, 7022, 7160,        INTERNET STANDARD
            7164, 8083, 8108                                Errata Exist
...
RFC 3550                          RTP                          July 2003
   ...
   marker (M): 1 bit
      ...
      标记的解释由配置文件定义。
      允许在包流中标记诸如帧边界之类的重要事件。
      配置文件可以定义附加的标志位 marker
      或通过改变有效载荷类型 payload 字段中的位数
      来指定不存在标志位（见5.3节）。
   ...
   sequence number: 16 bits
      ...
      对于发送的每个RTP数据包，序列号递增1，

      并且可以被接收器用来检测包丢失并恢复包序列。
      序列号的初始值应该是随机的（不可预测的），
      以使对加密的已知明文攻击更加困难，
      即使源本身未按照9.1节中的方法进行加密，
      数据包也可能流经进行加密的转换器。
      在[17]中讨论了选择不可预测数字的技术。
   ...
   timestamp: 32 bits
      ...
      采样时刻必须来自时钟，该时钟在时间上单调且线性地递增，
      以进行同步和抖动计算（请参见第6.4.1节）。
      时钟的分辨率必须足以满足所需的同步精度和测量数据包到达的抖动
     （每个视频帧一个滴答通常是不够的）。
      时钟频率取决于作为有效载荷携带的数据的格式，
      并在定义该格式的配置文件或有效载荷格式规范中静态指定，
      或者可以为通过非RTP方式定义的有效载荷格式动态指定。
      如果定期生成RTP数据包，则将使用从采样时钟确定的标称采样时刻，
      而不是读取系统时钟。
      例如，对于固定速率的音频，
      时间戳时钟可能会在输入设备中每160个采样周期内加一，
     对于每个这样的块，时间戳都会增加160，
      而无论该块是在包中传输还是被丢弃。
...
RFC 3550                          RTP                          July 2003
      ...
      序列号，时间戳的初始值应该是随机的。
      如果几个连续的RTP数据包是一次（逻辑上）生成的（属于同一视频帧），
      则它们将具有相等的时间戳。
      如果未按照采样顺序传输数据，则连续的RTP数据包可能包含非单调的时间戳，
      例如MPEG内插视频帧。（但发送的数据包的序列号 seq 仍然是单调的。）
